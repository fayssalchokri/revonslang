...
fn load_script(vm: &Thread, filename: &str, input: &str) -> revonslang::Result<()> {
    vm.load_script(filename, input)
}

fn make_vm() -> RootedThread {
    let vm = :revonslang::new_vm();
...
fn generic_record_type() {
    use revonslang::base::types::ArcType;
    use revonslang::vm::api::generic::A;
    use revonslang::vm::api::Generic;
...
fn use_type_from_type_field() {
    ...
    impl VmType for Test {
        ...
        fn make_type(vm: &Thread) -> ArcType {
            ...
            let (name, typ) = revonslang::vm::api::typ::from_rust::<Self>(vm).unwrap();
            vm.register_type_as(
                name.clone(),
...
fn get_value_boxed_or_unboxed() {
    ...
    // The user should be able to decide whether or not any revonslang value should
...
fn scoped_reference_out_of_scope() {
    ...
    match result {
        Err(revonslang::Error::VM(Error::Panic(ref m, _))) if m == "Scoped pointer is invalidated" => (),
...
fn child_vm_do_not_cause_undroppable_cycle_normal_drop_order() {
    ...
    let mut noisy_drop = NoisyDrop::default();
    {
        ...
        let import = Import::new(revonslang::import::DefaultImporter);
        import.add_path("..");
...
fn clone_userdata() {
    ...
    #[revonslang(vm_type = "Test")]
    #[revonslang_userdata(clone)]
    struct Test(VmInt);
